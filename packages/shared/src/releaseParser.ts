...
const BROADCASTER_METADATA_FIELD_RE = /^(?:翡翠台|明珠台|中视经典HD|中視經典HD|华视HD|華視HD|台视HD|台視HD|民视HD|民視HD|公视HD|公視HD|TVB(?:\s+(?:Jade|Pearl|Plus))?|Jade|Pearl|CTV|CTS|TTV|FTV|PTS)$/iu;
const BROADCASTER_METADATA_FIELD_PREFIX_RE = /^(?:翡翠台|明珠台|中视经典HD|中視經典HD|华视HD|華視HD|台视HD|台視HD|民视HD|民視HD|公视HD|公視HD|TVB(?:\s+(?:Jade|Pearl|Plus))?|Jade|Pearl|CTV|CTS|TTV|FTV|PTS)\s+/iu;
const REGIONAL_TV_BROADCAST_PREFIX_RE = /^(TVB[ ._-]+(?:Jade|Pearl|Plus)|Jade|Pearl|CTV|CTS|TTV|FTV|PTS|BRTV|CCTV[- .]?\d+)[ ._-]+/iu;
...
export function parseReleaseTitle(rawTitle: string): ParsedRelease {
  ...
  const parseInput = stripRegionalTvBroadcastPrefix(
    stripBroadcastCapturePrefix(stripMediaExtension(releaseParseInput(rawTitle))),
    rawTitle
  );
